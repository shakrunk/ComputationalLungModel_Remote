%% Computational Lung Model (ver 4)
...
fprintf("<strong># File: Lung_4.m</strong>\n");                             %lgf
fprintf("<strong># Title: Computational Lung Model (ver 4)</strong>\n");    %lgf
...
for i = 1:1:length(t)
    ...
    dPdt_O2_alv(i) = SA*D_O2*B_O2 * ( (P_O2_cap(i) - P_O2_alv(i))/l );
    dPdt_CO2_alv(i) = SA*D_CO2*B_CO2 * ( (P_CO2_cap(i) - P_CO2_alv(i))/l );
    dPdt_O2_cap(i) = (SA*D_O2*B_O2 * ( (P_O2_alv(i) - P_O2_cap(i))/l )) + (((P_O2_BIn - P_O2_cap(i))*Q_B)/V_B);
    dPdt_CO2_cap(i) = (SA*D_CO2*B_CO2 * ( (P_CO2_alv(i) - P_CO2_cap(i))/l )) + (((P_CO2_BIn - P_CO2_cap(i))*Q_B)/V_B);
